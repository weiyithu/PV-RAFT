...
from model.minkowski.modules.common import ConvType, NormType, conv, conv_tr, get_norm, get_nonlinearity_fn
...
class Res16UNetBase_cotrain(ResNetBase):
  BLOCK = None
  PLANES = (32, 64, 128, 256, 256, 256, 256, 256)
  DILATIONS = (1, 1, 1, 1, 1, 1, 1, 1)
  LAYERS = (2, 2, 2, 2, 2, 2, 2, 2)
  ...
  # Once data is processed, call clear to reset the model before calling initialize_coords
  def __init__(self, in_channels, out_channels, config, D=3, **kwargs):
    super(Res16UNetBase_cotrain, self).__init__(in_channels, out_channels, config, D)
  ...
  def network_initialization(self, in_channels, out_channels, config, D):
    ...
    self.feature_embedding = conv(self.PLANES[7], out_channels, kernel_size=1, stride=1, bias=True, D=D)
    # self.feature_embedding = nn.Sequential(
    #           conv(self.PLANES[7], out_channels, kernel_size=1, stride=1, bias=True, D=D),
    #           ME.MinkowskiReLU(),
    #           conv(out_channels, out_channels, kernel_size=1, stride=1, bias=True, D=D))
  ...
  def forward(self, x):
    ...
    out = me.cat(out, out_p1)
    out = self.block8(out)
    out = self.feature_embedding(out)
    return out
...
class Res16UNet34_cotrain(Res16UNetBase_cotrain):
  BLOCK = BasicBlock
  LAYERS = (2, 3, 4, 6, 2, 2, 2, 2)
class Res16UNet34C_cotrain(Res16UNet34_cotrain):
  PLANES = (32, 64, 128, 256, 256, 128, 96, 96)
